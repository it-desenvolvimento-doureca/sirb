...
@Component({
  selector: 'app-objetivos-cumprimento-fornecedor',
  templateUrl: './objetivos-cumprimento-fornecedor.component.html',
  styleUrls: ['./objetivos-cumprimento-fornecedor.component.css']
})
export class ObjetivosCumprimentoFornecedorComponent implements OnInit {
  ID: number;
    ...
    this.LGDICOBJETIVOSService.getAll().subscribe(
      response => {
        for (var x in response) {
          ...
          this.objetivo = response[x].objetivo_FORNECEDORES;
        }
  ...
  gravar() {
    ...
    OBJETIVO.id = this.ID;
    OBJETIVO.objetivo_FORNECEDORES = this.objetivo;
