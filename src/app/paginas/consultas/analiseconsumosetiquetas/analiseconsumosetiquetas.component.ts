...
@Component({
  selector: 'app-analiseconsumosetiquetas',
  templateUrl: './analiseconsumosetiquetas.component.html',
  styleUrls: ['./analiseconsumosetiquetas.component.css']
})
export class AnaliseconsumosetiquetasComponent {
  ...
  constructor(private ABDICTIPOMANUTENCAOService: ABDICTIPOMANUTENCAOService, private renderer: Renderer, private confirmationService: ConfirmationService, private ABMOVMANUTENCAOService: ABMOVMANUTENCAOService, private GERVISTASService: GERVISTASService) {
    ...
    this.disEditar = !JSON.parse(localStorage.getItem('acessos')).find(item => item.node == "node052editar");
    this.disGravar = !JSON.parse(localStorage.getItem('acessos')).find(item => item.node == "node052gravar");
    this.disCriar = !JSON.parse(localStorage.getItem('acessos')).find(item => item.node == "node052criar");
    ...
    //carregas vistas
    this.GERVISTASService.getbyid_pagina(4).subscribe(
      response => {
        ...
        if (total > 0) {
          ...
          for (var y in response) {
            ...
            */
            ...
            rowData['CONVERSOR'] = (response[y][41] != null && response[y][41] != "" && response[y][41] != 0) ? parseFloat(response[y][41]) : 1;
