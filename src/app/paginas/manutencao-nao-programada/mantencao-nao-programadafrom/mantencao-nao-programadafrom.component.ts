...
@Component({
  selector: 'app-mantencao-nao-programadafrom',
  templateUrl: './mantencao-nao-programadafrom.component.html',
  styleUrls: ['./mantencao-nao-programadafrom.component.css']
})
export class MantencaoNaoProgramadafromComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.globalVar.setdisDuplicar(!JSON.parse(localStorage.getItem('acessos')).find(item => item.node == "node004duplicar"));

    this.disimprimir = !JSON.parse(localStorage.getItem('acessos')).find(item => item.node == "node004imprimir");
    this.disimprimiretiquetas = !JSON.parse(localStorage.getItem('acessos')).find(item => item.node == "node004imprimiretiquetas");
    ...
    if (urlarray[1].match("editar") || urlarray[1].match("view")) {
      ...
      if (this.globalVar.getfiltros("manutencaonaoprogramada_id") && this.globalVar.getfiltros("manutencaonaoprogramada_id").length > 0) {
        this.manutencao = this.globalVar.getfiltros("manutencaonaoprogramada_id");
      ...
      this.ABDICBANHOADITIVOService.getbyID_banho(event.value.id).subscribe(
        response => {
          ...
          if (count > 0) {
            ...
            for (var x in response) {
              var disabled = true;
              if (response[x][0].manutencaonaoprogramada == true) disabled = false;
  ...
  carregaaditivosstock(response, x, pos, pos2, total2, array, disabled) {
    var query = [];
    var total = null;
  ...
  preenche_banhos(id) {
    ...
    if (this.linha["id"] != "") {
      //preenche combobox banhos
      this.ABDICBANHOService.getAllLINHAbylinha_tipo(this.linha["id"], "N").subscribe(
        response => {
    ...
    if (!this.planeamento && this.novo) {
      ...
      this.ABMOVMANUTENCAOService.create(MOV_MANUTENCAO).subscribe(
        res => {
          ...
          if (this.url != null) {
            this.router.navigate(['manutencaonaoprogramada/editar'], { queryParams: { id: res.id_MANUTENCAO, redirect: "listagem" } });
          } else {
            this.router.navigate(['manutencaonaoprogramada/editar'], { queryParams: { id: res.id_MANUTENCAO } });
          }
        }, error => {
          console.log(error); this.simular(this.inputerro);
        });
    ...
    this.ABMOVMANUTENCAOLINHAService.update(MOV_MANUTENCAO_LINHA).then(() => {
      if (id_manu != null && count == (parseInt(x) + 1)) {
        this.ABMOVMANUTENCAOLINHAService.apagar_linhas(id_manu).then(() => {
          ...
          console.log(error); this.simular(this.inputerro); this.btgravar = false;
        });
      }
    }, error => {
      console.log(error); this.simular(this.inputerro); this.btgravar = false;
  ...
  seguinte() {
    ...
    if (this.manutencao.length > 0) {
      ...
      if (this.url != null) {
        this.router.navigate(['manutencaonaoprogramada/view'], { queryParams: { id: this.manutencao[this.i], redirect: "listagem" } });
      } else {
        this.router.navigate(['manutencaonaoprogramada/view'], { queryParams: { id: this.manutencao[this.i] } });
      }
    }
  }
  ...
  concluir() {
    ...
    if (this.arrayForm[0].id == null) {
      ...
      if (encontrou) {
        ...
        this.ABMOVMANUTENCAOService.update(MOV_MANUTENCAO).then(() => {
          this.router.navigate(['manutencaonaoprogramada']);
          this.simular(this.inputgravou);
        }, error => {
          console.log(error); this.simular(this.inputerro);
        });
    ...
    this.ABMOVMANUTENCAOCABService.getbyID(id).subscribe(
      response => {
        ...
        if (count > 0) {
          ...
          if (this.url != null) {
            this.router.navigate(['manutencaonaoprogramada/editar'], { queryParams: { id: this.id, redirect: "listagem" } });
          } else {
            this.router.navigate(['manutencaonaoprogramada/editar'], { queryParams: { id: this.id } });
          }
      ...
      resp => {
        ...
        if (count > 0) {
          for (var x in resp) {
            ...
            var adi = this.arrayForm.find(item => item.pos == pos).aditivos.find(item => item.id_LIN == resp[x][2]);
            if (adi) {
              if (parseInt(resp[x][1]) == 0 && resp[x][0].cisterna && adi.valor1 != null && adi.valor1 != "" && adi.valor1 != 0) {
                encontrou2 = true;
  ...
  confirmar_linha1(pos, id, id_manu) {
    this.arrayForm.find(item => item.pos == pos).executado = false;
    var encontrou = false;
    for (var x in this.arrayForm.find(item => item.pos == pos).aditivos) {
  ...
  preenchetipo_man(val, id) {
    ...
    this.ABDICTIPOMANUTENCAOService.getAll(["N"]).subscribe(
      response => {
        this.tipo_manu = [];
  ...
  preencheAdicao(val, id) {
    ...
    this.ABDICTIPOADICAOService.getAll(["N"]).subscribe(
      response => {
        this.tipo_adicao = [];
  ...
  preencheIntervalo(val, id) {
    ...
    this.ABDICTIPOOPERACAOService.getAll(["N"]).subscribe(
      response => {
        this.intervalo_op = [];
  ...
  historico(id) {
    this.router.navigate(['manutencaonaoprogramada/historico'], { queryParams: { id: id, classif: 'N' } });
  }
