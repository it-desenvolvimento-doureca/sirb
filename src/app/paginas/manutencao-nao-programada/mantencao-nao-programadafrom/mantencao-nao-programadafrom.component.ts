      ...
      respo => {
        ...
        if (count > 0) {
          for (var x in respo) {
            ...
            var adi = this.arrayForm.find(item => item.pos == pos).aditivos.find(item => item.id_LIN == respo[x][2]);
            if (adi) {
              if (parseInt(respo[x][1]) == 0 && respo[x][0] && adi.valor1 != null && adi.valor1 != "" && adi.valor1 != 0) {
                encontrou2 = true;
