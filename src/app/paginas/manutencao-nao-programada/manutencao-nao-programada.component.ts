...
@Component({
  selector: 'app-manutencao-nao-programada',
  templateUrl: './manutencao-nao-programada.component.html',
...
export class ManutencaoNaoProgramadaComponent implements OnInit {
  ...
  ngOnInit() {
    this.filtroval = true;
    var array = this.globalVar.getfiltros("manutencaonaoprogramada");
    if (array) {
    ...
    this.cols = [];
    this.estados = [{ label: "Em Execução", value: "Em Execução" }, { label: "Executado", value: "Executado" }];
  ...
  carregarlista() {
    ...
    if (count == 0) {
      ...
      this.pesquisarbanhos(this.globalVar.getfiltros("manutencaonaoprogramadaidbanho"));
    }
  }
  ...
  reset() {
    ...
    var count = 0;
    if (this.globalVar.getfiltros("manutencaonaoprogramadaidbanho")) count = 1;
    if (count > 0) {
      this.globalVar.setfiltros("manutencaonaoprogramadaidbanho", null);
      this.carregarlista();
    }
  }
  ...
  filtrar(value, coluna, fil = false, filtro = "contains") {
    ...
    this.globalVar.setfiltros("manutencaonaoprogramada", this.dataTableComponent.filters);
    var ids = [];
  ...
  abrir(event) {
    this.router.navigate(['manutencaonaoprogramada/view'], { queryParams: { id: event.data.id } });
  }
  ...
  pesquisarbanhos(id) {
    if (id.id) {
      var ids = [];
      this.cols = [];
      this.ABMOVMANUTENCAOService.getAllid_banho(this.query, "N", id.id).subscribe(
        response => {
          var ids = [];
          for (var x in response) {
            this.cols.push({
  ...
  preenche_banhos() {
    ...
    this.ABDICBANHOService.getAllLINHAbylinhatodos(linha).subscribe(
      response => {
        ...
        var count = 0;
        if (this.globalVar.getfiltros("manutencaonaoprogramadaidbanho")) count = 1;
        if (count > 0) this.banho = this.globalVar.getfiltros("manutencaonaoprogramadaidbanho");
      },
      error => console.log(error));
  }
}
