...
export class ConfiguracoesComponent implements OnInit {
  ...
  carregaSectores() {
    ...
    var array = [{ id: 0, parent: null, name: 'Sectores', children: [] }];


    this.RHSECTORESService.getAll().subscribe(
      response => {
        for (var x in response) {
          array[0].children.push({ id: response[x][0].cod_SECTOR, parent: 0, name: response[x][0].des_SECTOR, children: [] })
        }
        this.list2 = array;
      },
      error => console.log(error));
  ...
  carregaArmazens() {
    ...
    var array = [{ id: 0, parent: null, name: 'Armazéns', children: [] }];


    this.GERARMAZEMService.getAll_silver().subscribe(
      response => {
        for (var x in response) {
          array[0].children.push({ id: response[x].LIECOD, parent: 0, name: response[x].LIECOD + ' - ' + response[x].ADRNOM, children: [] })
        }
        this.list3 = array;
      },
      error => console.log(error));
  ...
  //ver utilizadores do perfil
  carregaperfis() {
    this.sourcePerfil = [];
    this.GERUTZPERFILService.getAllbyidnotuser(this.id_perfil, this.id_modulo).subscribe(
      response => {
        this.sourcePerfil = [];
        for (var x in response) {
          this.sourcePerfil.push({ nome: response[x][1], id: response[x][0] });
        }
    ...
    this.targetPerfil = [];
    this.GERUTZPERFILService.getAllbyiduser(this.id_perfil, this.id_modulo).subscribe(
      response => {
        this.targetPerfil = [];
        for (var x in response) {
          this.targetPerfil.push({ nome: response[x][1], id: response[x][0], id_perfil: response[x][2] });
        }
  ...
  onMoveToTarget(e) {
    ...
    for (x in e.items) {
      ...
      perf.id_PERFIL = this.id_perfil;
      perf.id_UTZ = e.items[x].id;
  ...
  onMoveToSource(e) {
    var x;
    for (x in e.items) {
      this.GERUTZPERFILService.delete(e.items[x].id_perfil).then(() => {
