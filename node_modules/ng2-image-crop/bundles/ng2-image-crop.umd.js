(function (global, factory) {
	...
	(factory((global['ng2-image-crop'] = {}),global.ng.core,global.ng.common,global.ng.platformBrowser));
...
function resetExifOrientation(srcBase64) {
    try {
        var /** @type {?} */ exifRotation = getExifRotation(srcBase64);
        if (exifRotation > 1) {
...
function getExifRotation(imageBase64) {
    var /** @type {?} */ view = new DataView(base64ToArrayBuffer(imageBase64));
...
function resizeCanvas(canvas, width, height, resizeCanvas) {
    ...
    var /** @type {?} */ width_source = canvas.width;
    var /** @type {?} */ height_source = canvas.height;
    width = Math.round(width);
    height = Math.round(height);
    var /** @type {?} */ ratio_w = width_source / width;
    var /** @type {?} */ ratio_h = height_source / height;
    var /** @type {?} */ ratio_w_half = Math.ceil(ratio_w / 2);
    var /** @type {?} */ ratio_h_half = Math.ceil(ratio_h / 2);
    var /** @type {?} */ ctx = canvas.getContext('2d');
    if (ctx) {
        var /** @type {?} */ img = ctx.getImageData(0, 0, width_source, height_source);
        var /** @type {?} */ img2 = ctx.createImageData(width, height);
        var /** @type {?} */ data = img.data;
        var /** @type {?} */ data2 = img2.data;
        for (var /** @type {?} */ j = 0; j < height; j++) {
            for (var /** @type {?} */ i = 0; i < width; i++) {
                var /** @type {?} */ x2 = (i + j * width) * 4;
                var /** @type {?} */ center_y = j * ratio_h;
                var /** @type {?} */ weight = 0;
                var /** @type {?} */ weights = 0;
                var /** @type {?} */ weights_alpha = 0;
                var /** @type {?} */ gx_r = 0;
                var /** @type {?} */ gx_g = 0;
                var /** @type {?} */ gx_b = 0;
                var /** @type {?} */ gx_a = 0;
                var /** @type {?} */ xx_start = Math.floor(i * ratio_w);
                var /** @type {?} */ yy_start = Math.floor(j * ratio_h);
                var /** @type {?} */ xx_stop = Math.ceil((i + 1) * ratio_w);
                var /** @type {?} */ yy_stop = Math.ceil((j + 1) * ratio_h);
                xx_stop = Math.min(xx_stop, width_source);
                yy_stop = Math.min(yy_stop, height_source);
                for (var /** @type {?} */ yy = yy_start; yy < yy_stop; yy++) {
                    var /** @type {?} */ dy = Math.abs(center_y - yy) / ratio_h_half;
                    var /** @type {?} */ center_x = i * ratio_w;
                    var /** @type {?} */ w0 = dy * dy; //pre-calc part of w
                    for (var /** @type {?} */ xx = xx_start; xx < xx_stop; xx++) {
                        var /** @type {?} */ dx = Math.abs(center_x - xx) / ratio_w_half;
                        var /** @type {?} */ w = Math.sqrt(w0 + dx * dx);
                        if (w >= 1) {
                            //pixel too far
                            continue;
                        }
                        ...
                        var /** @type {?} */ pos_x = 4 * (xx + yy * width_source);
                        //alpha
...
function fitImageToAspectRatio(srcBase64, aspectRatio) {
    return new Promise(function (resolve, reject) {
        ...
        img.onload = function () {
            ...
            var /** @type {?} */ ctx = canvas.getContext('2d');
            if (ctx) {
                var /** @type {?} */ minWidthToContain = img.height * aspectRatio;
                var /** @type {?} */ minHeightToContain = img.width / aspectRatio;
                canvas.width = Math.max(img.width, minWidthToContain);
                canvas.height = Math.max(img.height, minHeightToContain);
                var /** @type {?} */ dx = (canvas.width - img.width) / 2;
                var /** @type {?} */ dy = (canvas.height - img.height) / 2;
...
var ImageCropperComponent = (function () {
    ...
    ImageCropperComponent.prototype.resizeCropperPosition = function () {
        var /** @type {?} */ sourceImageElement = this.sourceImage.nativeElement;
    ...
    ImageCropperComponent.prototype.resetCropperPosition = function () {
        ...
        if (!this.maintainAspectRatio) {
            ...
            this.cropper.x1 = 0;
            this.cropper.x2 = sourceImageElement.offsetWidth;
            var /** @type {?} */ cropperHeight = sourceImageElement.offsetWidth / this.aspectRatio;
            ...
            this.cropper.y1 = 0;
            this.cropper.y2 = sourceImageElement.offsetHeight;
            var /** @type {?} */ cropperWidth = sourceImageElement.offsetHeight * this.aspectRatio;
    ...
    ImageCropperComponent.prototype.crop = function (outputType) {
        ...
        if (this.sourceImage.nativeElement && this.originalImage != null) {
            ...
            var /** @type {?} */ imagePosition = this.getImagePosition();
            var /** @type {?} */ width = imagePosition.x2 - imagePosition.x1;
            var /** @type {?} */ height = imagePosition.y2 - imagePosition.y1;
            ...
            if (ctx) {
                ...
                var /** @type {?} */ output = { width: width, height: height, imagePosition: imagePosition, cropperPosition: Object.assign({}, this.cropper) };
                var /** @type {?} */ resizeRatio = this.getResizeRatio(width, height);
                if (resizeRatio !== 1) {
    ...
    ImageCropperComponent.prototype.getImagePosition = function () {
        var /** @type {?} */ sourceImageElement = this.sourceImage.nativeElement;
        var /** @type {?} */ ratio = this.originalSize.width / sourceImageElement.offsetWidth;
        return {
    ...
    ImageCropperComponent.prototype.cropToBase64 = function (cropCanvas) {
        var /** @type {?} */ imageBase64 = cropCanvas.toDataURL('image/' + this.format, this.getQuality());
...
//# sourceMappingURL=ng2-image-crop.umd.js.map
