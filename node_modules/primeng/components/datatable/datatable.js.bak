...
var DataTable = (function () {
    function DataTable(el, domHandler, renderer, changeDetector, objectUtils) {
        ...
        this.emptyMessage = 'Nenhum registro foi encontrado';
        ...
        this.filterConstraints = {
            ...
            contains: function (value, filter) {
                ...
                return removeDiacritics(value.toString().toLowerCase()).indexOf(removeDiacritics(filter.toLowerCase())) !== -1;
            },
            ...
            in: function (value, filter) {
				...
				if (removeDiacritics(value.toString().toLowerCase()).indexOf(removeDiacritics(filter[i].toLowerCase())) !== -1)
					return true;
                }
                return false;
            }
...
<<<<<<< HEAD
		...
		if(this.rows_name == "todos"){this.rows = this.value.length;}
		//total records
=======
        //total records
...
<<<<<<< HEAD
        DataTable.first = 0;          
		this.rows_name = event.rows;
			
			
		if(event.rows == "todos"){
			this.rows = this.value.length;				
			this._first = 0; 
			this.firstChange.emit(0);
		}else{
...
=======
		...
		if(event.rows == "todos"){this.rows = this.totalRecords; this._first = 0;}
>>>>>>> aa167a7d63b9fa01b26efb1fceaeb7aed3e4b2ea
        this.paginate();
    };
...
<<<<<<< HEAD
			...
			//if(this.rows_name == "todos"){this._first = 1;}
        ...
        if (this.lazy) {
            ...
            for (var i = 0; i < this.value.length; i++) {
                ...
                for (var j = 0; j < this.columns.length; j++) {
                    ...
                    //local
                    if (filterMeta) {
                        var filterValue = filterMeta.value, filterField = col.field, filterMatchMode = filterMeta.matchMode || 'contains', dataFieldValue = this.resolveFieldData(this.value[i], filterField);
                        var filterConstraint = this.filterConstraints[filterMatchMode];
                        if (!filterConstraint(dataFieldValue, filterValue)) {/*tiago*/
                            localMatch = false;
                        }
                        if (!localMatch) {
                            break;
